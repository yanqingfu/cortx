...
def get_channels(client, limit=None):
    channels = {}
    response = call_api(client=client, method="conversations.list",
...
def get_member_count(client, channels):
    ...
    for cid, cname in channels.items():
        response = call_api(client=client, method="conversations.info",
                            data={'channel': cid, 'include_num_members': 'true'})
    ...
    for m in request['members']:
        ...
        if slack_people.find_person(m['id']):
            ...
            try:
                ...
                print("Person %s [%s] (%s, %s) %s already in our people pickle" % (
                    real_name, presence, m['name'], email, 'is' if found else 'is not'), found)
...
def get_channel_conversations(client, channel, cname):
    ...
    while True:
        ...
        response = call_api(client=client, method='conversations.history',
                            data={'channel': channel, 'limit': MESSAGES_PER_PAGE, 'cursor': cursor})
